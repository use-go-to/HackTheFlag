...
(function (global) {
  ...
  // ============================ SYSTEME DE FICHIERS =========================
  // Un noeud "d" (dir) a { type:'d', children:{name:node}, owner, perms }
  // Un noeud "f" (file) a { type:'f', content:'...', owner, perms, size }
  // perms : chaîne de 6 caractères "rw-r--" = [owner-r,owner-w,other-r,other-w]
  // (on simplifie volontairement : pas de bit x séparé pour les fichiers,
  //  pas de notion de groupe — largement suffisant pour l'usage pédagogique,
  //  et déjà bien plus riche qu'un simple sac de chaînes figées.)
  function dirNode(children, meta) {
    return Object.assign({ type: "d", children: children || {} }, meta || {});
  }
  function fileNode(content, meta) {
    const c = content == null ? "" : content;
    return Object.assign({ type: "f", content: c, size: c.length }, meta || {});
  }
  ...
  function canRead(node, user) {
    ...
    if (!node.perms) return true; // pas de restriction définie -> lisible
    if (node.owner === user) return node.perms[0] === "r";
    return node.perms[2] === "r";
  }
  function canEnter(node, user) {
    // Pour un dossier : on réutilise la même logique que canRead (simplifiée).
    return canRead(node, user);
  }
  ...
  function PentestSimEngine(course) {
    this.course = course;
    this.state = {
      hostId: course.attacker.id,     // machine "active" dans le terminal
      user: course.attacker.user,
      cwd: course.attacker.home,
      events: new Set(),              // évènements bruts observés (source de vérité du score)
      discovered: {},                 // hostId -> {ports:bool, web:bool}
      findings: [],                   // pastilles "trouvailles" affichées en tête de terminal
      findingKeys: new Set(),
      hints: {},                      // objId -> nb d'indices utilisés
      solutions: {},                  // objId -> bool solution révélée
      flagValidated: false,
      sshAttempts: 0,
      lockedUntil: 0
    };
  ...
  PentestSimEngine.prototype.cwdObj = function () {
    const h = this.host();
    return { path: this.state.cwd, home: (h.users && h.users[this.state.user] && h.users[this.state.user].home) || "/" };
  };

  PentestSimEngine.prototype.event = function (key) {
    if (!this.state.events.has(key)) {
      this.state.events.add(key);
      this.onEventAdded && this.onEventAdded(key);
    }
  };
  PentestSimEngine.prototype.hasEventPrefix = function (prefix) {
    for (const e of this.state.events) { if (e.indexOf(prefix) === 0) return true; }
    return false;
  };
  ...
  PentestSimEngine.prototype.promptParts = function () {
    ...
    let shown = this.state.cwd === home ? "~" : (this.state.cwd.indexOf(home + "/") === 0 ? "~" + this.state.cwd.slice(home.length) : this.state.cwd);
    return { user: u, host: h.hostname, path: shown, sym: u === "root" ? "#" : "$" };
  };

  // ============================== EXECUTION SHELL ============================
  // Retourne un tableau de lignes {t, c} où c ∈ '' | 'ok' | 'err' | 'flag' | 'dim'
  PentestSimEngine.prototype.exec = async function (raw) {
    ...
    if (!Array.isArray(lines)) return lines; // résultat spécial (ssh multi-étapes, exit distant…)
  ...
  PentestSimEngine.prototype.applyPipe = function (stage, lines) {
    ...
    if (cmd === "grep") {
      let i = ("caseInsensitive" in {}) ? 1 : 1;
      let ci = false, pattern = null;
      const rest = argv.slice(1);
      const filtered = [];
      rest.forEach(function (a) { if (a === "-i") ci = true; else filtered.push(a); });
      pattern = filtered[0];
    ...
    if (cmd === "wc" && argv[1] === "-l") {
      return [{ t: String(texts.length), c: "o" }];
    }
    if (cmd === "sort") {
      return texts.slice().sort().map(function (t) { return { t: t, c: "o" }; });
    }
    if (cmd === "head") {
  ...
  PentestSimEngine.prototype.execOne = async function (raw) {
    ...
    const cmd = (argv[0] || "").toLowerCase();
    const args = argv.slice(1).filter(function (a) { return !/^[12]>(\/dev\/null)?$/.test(a); }); // avale les redirections d'erreur factices
    ...
    // commandes spécifiques au cours (exploits particuliers, outils custom)
  ...
  PentestSimEngine.prototype.cmd_id = function () {
    ...
    return [{ t: this.state.user === "root" ? "uid=0(root) gid=0(root) groups=0(root)" : ("uid=1000(" + this.state.user + ") gid=1000(" + this.state.user + ")"), c: "o" }];
  };
  ...
  PentestSimEngine.prototype.cmd_ls = function (args) {
    ...
    if (!canEnter(node, this.state.user)) { this.event("denied:" + h.id + ":" + abs); return [{ t: "ls: impossible d'ouvrir le dossier '" + (target || ".") + "': Permission non accordée", c: "err" }]; }
    ...
    const out = [{ t: "total " + (names.length * 4), c: "o" }];
    names.forEach(function (n) {
      const c = node.children[n];
      const perms = c.type === "d" ? "d" : "-";
      const p = c.perms || "rw-r--";
      const rwx = perms + p[0] + p[1] + "-" + (perms === "d" ? "x" : "-") + p[2] + p[3] + "-" + (perms === "d" ? "x" : "-");
      const owner = c.owner || "root";
      const size = c.type === "f" ? c.size : 4096;
      out.push({ t: rwx.padEnd(11) + " 1 " + owner.padEnd(6) + owner.padEnd(6) + String(size).padStart(6) + " " + (c.mtime || "Jan  1 00:00") + " " + n + (c.type === "d" ? "/" : ""), c: (c.suid ? "flag" : "o") });
    });
    return out;
  };
  ...
  PentestSimEngine.prototype.cmd_cd = function (args) {
    ...
    if (!canEnter(node, this.state.user)) { this.event("denied:" + h.id + ":" + abs); return [{ t: "bash: cd: " + target + ": Permission non accordée", c: "err" }]; }
    this.state.cwd = abs;
    return [];
  };
  ...
  PentestSimEngine.prototype.cmd_find = function (args) {
    ...
    const wantSuid = args.indexOf("-perm") !== -1 && (args.indexOf("-4000") !== -1);
    ...
    (function walk(node, abs) {
      if (!node) return;
      if (!canEnter(node, self.state.user)) return; // find respecte les permissions, comme le vrai find
      if (node.type === "d") {
        Object.keys(node.children).forEach(function (name) { walk(node.children[name], abs === "/" ? "/" + name : abs + "/" + name); });
        return;
      }
      let ok = true;
  ...
  // ------------------------------------ sudo -----------------------------------
  PentestSimEngine.prototype.cmd_sudo = function (args, raw) {
    const h = this.host();
    if (args[0] === "-l") {
  ...
  // ------------------------------------ gobuster / dirb -------------------------
  PentestSimEngine.prototype.cmd_gobuster = function (args) {
    return this._webBrute(args, "gobuster");
  };
  PentestSimEngine.prototype.cmd_dirb = function (args) {
    return this._webBrute(args, "dirb");
  };
  PentestSimEngine.prototype._webBrute = function (args, tool) {
    ...
    const out = [{ t: "===============================================================", c: "dim" }, { t: (tool === "gobuster" ? "Gobuster v3.8.2" : "DIRB v2.22"), c: "dim" }, { t: "===============================================================", c: "dim" }];
  ...
  PentestSimEngine.prototype.cmd_wget = function (args) {
    ...
    if (cwd && cwd.type === "d") cwd.children[fname] = fileNode(r.content, { owner: this.course.attacker.user, perms: "rwrw", mtime: "aujourd'hui" });
    return [
  ...
  PentestSimEngine.prototype.cmd_ssh = function (args) {
    ...
    if (!m) return [{ t: "usage: ssh utilisateur@ip", c: "err" }];
    const user = m[1], ip = m[2];
    const targetHost = Object.values(this.hosts).find(function (h) { return h.ip === ip; });
    if (!targetHost) return [{ t: "ssh: connect to host " + ip + " port 22: No route to host", c: "err" }];
    if (!targetHost.users || !targetHost.users[user]) {
      return [{ t: "ssh: connect to host " + ip + " port 22: Connection refused", c: "err" }];
    }
    this._pendingSsh = { targetHost, user, stage: "confirm" };
    return {
      lines: [
  ...
  // Suite d'une session ssh en cours (confirmation d'empreinte puis mot de passe)
  PentestSimEngine.prototype.continueSsh = async function (input) {
    ...
    if (p.stage === "confirm") {
      ...
      p.stage = "password";
      return { lines: [{ t: "Warning: Permanently added '" + p.targetHost.ip + "' (ED25519) to the list of known hosts.", c: "o" }], prompt: { label: p.user + "@" + p.targetHost.ip + "'s password:", mask: true } };
    }
    if (p.stage === "password") {
      const now = Date.now();
      ...
      const expected = p.targetHost.users[p.user].password;
      if (input === expected) {
        this.state.sshAttempts = 0;
        this.state.hostId = p.targetHost.id;
        this.state.user = p.user;
        this.state.cwd = p.targetHost.users[p.user].home;
  ...
  // ------------------------------------ su / privesc ---------------------------------
  PentestSimEngine.prototype.cmd_su = function (args) {
    return [{ t: "su: Authentication failure", c: "err" }];
  };
  PentestSimEngine.prototype.cmd_exit = function () {
    if (this.state.hostId !== this.course.attacker.id) {
      const closedIp = this.host().ip;
      this.state.hostId = this.course.attacker.id;
      this.state.user = this.course.attacker.user;
      this.state.cwd = this.course.attacker.home;
      return { lines: [{ t: "logout", c: "o" }, { t: "Connection to " + closedIp + " closed.", c: "o" }], newContext: true };
    }
    return [{ t: "exit", c: "dim" }];
  };
  // Complétion : liste des sous-entrées visibles du dossier courant (respecte
  // les permissions), utilisée par l'UI pour compléter un chemin après Tab.
  PentestSimEngine.prototype.pathCandidates = function () {
    const h = this.host();
    const node = getNode(h.vfs, this.state.cwd);
  ...
  // élève réellement l'utilisateur courant plutôt que d'afficher un texte figé.
  PentestSimEngine.prototype.tryPrivesc = function (raw) {
    ...
    for (const rule of rules) {
      if (rule.match(raw, this.state)) {
        ...
        return rule.output || [{ t: "whoami", c: "dim" }, { t: this.state.user, c: "ok" }];
      }
    }
    return null;
  };
  ...
  // objectifs "coeur" fait 100). Un indice coûte 30% des points de CET
  // objectif, une solution en coûte 60% — jamais la totalité, jamais
  // globalement comme dans l'ancien moteur (-2/-6 fixes indifférents au
  // poids de l'étape). Des objectifs "bonus" (trouvailles optionnelles)
  // s'ajoutent par-dessus sans jamais faire dépasser 20/20.
  PentestSimEngine.prototype.computeScore = function () {
    ...
    core.forEach(function (obj) {
      ...
      if (done) {
        ...
        if (hints > 0) pts -= obj.points * Math.min(hints, 2) * 0.15; // -15%/indice, plafonné 2 indices
        if (self.state.solutions[obj.id]) pts = Math.min(pts, obj.points * 0.4); // solution révélée -> plafond 40%
        pts = Math.max(pts, obj.points * 0.15); // toujours un minimum si réellement validé
      }
      raw += pts;
      detail.push({ obj, done, pts });
    });
    ...
    note += (bonusPts / Math.max(maxRaw, 1)) * 20; // bonus exprimé dans la même échelle, non plafonné avant clamp
    if (this.state.flagValidated) note = Math.max(note, 4); // plancher annoncé une fois le flag validé
    note = Math.max(0, Math.min(20, note));
    return { note: Math.round(note * 10) / 10, raw, maxRaw, bonusPts, bonusMax, detail };
  };

  global.PentestSim = { Engine: PentestSimEngine, sha256Hex, safeEqual, resolvePath, getNode, dirNode, fileNode, tokenize };
})(window);
